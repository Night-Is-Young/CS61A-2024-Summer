...
class Ant(Insect):
    ...
    def group_attack(self,damage):
        bees_duplication,bees_result=self.place.bees[:],[]
        for bee in bees_duplication:
            if bee.health>damage:
                bees_result.append(bee)
            bee.reduce_health(damage)
        self.place.bees=bees_result
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        attack_place,distance=self.place,0
        while not attack_place.is_hive:
            if attack_place.bees and distance<=self.upper_bound and distance>=self.lower_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        total_damage=amount+(self.damage if self.health<=amount else 0)
        super().group_attack(total_damage)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return not(other.is_container or self.ant_contained!=None)
        # END Problem 8a
...
class TankAnt(ContainerAnt):
    ...
    def action(self,gamestate):
        ...
        super().group_attack(self.damage)
# END Problem 9
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while cur_place.exit!=None:
            ...
            cur_ant=cur_place.ant
            if cur_ant:
                if isinstance(cur_ant,ContainerAnt) and cur_ant.ant_contained:
        ...
        while not cur_place.is_hive:
            if cur_place.ant:
                if isinstance(cur_place.ant,ContainerAnt):
                    if cur_place.ant.ant_contained is not self:
                        insects_dic[cur_place.ant.ant_contained]=distance
                if cur_place.ant is not self:
