...
def pick(paragraphs, select, k):
    ...
    for i in range(len(paragraphs)):
        if select(paragraphs[i]):cnt+=1
        if cnt==k:return paragraphs[i]
    return ''
    # END PROBLEM 1
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def about_word(sentence):
        word_list=split(remove_punctuation(lower(sentence)))
        for word in subject:
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    cnt,typed_len,source_len=0,len(typed_words),len(source_words)
    if typed_len==0 and source_len==0:return 100.0
    elif typed_len==0 or source_len==0:return 0.0
    else:
        cnt=sum([1 for _ in range(min(typed_len,source_len)) if  typed_words[_]==source_words[_]])
        return cnt/typed_len*100
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        # Check if result is in cache and if limit is sufficient
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    correct_word,min_diff=typed_word,100000000
    for word in word_list:
        if word==typed_word:return word
        diff=diff_function(typed_word,word,limit)
        if diff<min_diff:
            min_diff=diff
            correct_word=word
    return correct_word if min_diff<=limit else typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    '''def autofix_func(t,s,num):
        if len(t)==0 or len(s)==0 or num>limit:return num
        else:return autofix_func(t[1:],s[1:],num+(1 if t[0]!=s[0] else 0))
    return autofix_func(typed,source,abs(len(typed)-len(source)))'''
    if limit<0:return limit+1
    elif len(typed)==0 or len(source)==0:return max(len(typed),len(source))
    else:
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if limit<0 or abs(len(typed)-len(source))>limit:return limit+1
    if typed==source:return 0
    if len(typed)==0 or len(source)==0:return max(len(typed),len(source))
    if typed[0]==source[0]:return minimum_mewtations(typed[1:],source[1:],limit)
    add_cost=minimum_mewtations(typed,source[1:],limit-1)
    del_cost=minimum_mewtations(typed[1:],source,limit-1)
    rep_cost=minimum_mewtations(typed[1:],source[1:],limit-1)
    return min(add_cost,del_cost,rep_cost)+1
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        time_player=timestamps_per_player[i]
        times.append([time_player[j+1]-time_player[j] for j in range(len(time_player)-1)])
    return words,times
    # END PROBLEM 9
...
def fastest_words(match_object):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            player_time=get_time(match_object,j,i)
            if player_time<min_time:
                min_time=player_time
                player_num=j
        fastest_words_list[player_num].append(get_all_words(match_object)[i])
