...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and': # and expressions
            ...
            return calc_apply(calc_eval(exp.first), exp.rest.map(calc_eval)) # UPDATE THIS FOR Q2, what is type(operator)?
...
def floor_div(args):
    ...
    """
    "*** YOUR CODE HERE ***"
    result,dividor=args.first,args.rest
    while dividor:
        result//=dividor.first
        dividor=dividor.rest
    return result
...
def eval_and(expressions):
    ...
    """
    "*** YOUR CODE HERE ***"
    cur_expr,cur_value=expressions,True
    while cur_expr:
        cur_value=calc_eval(cur_expr.first)
