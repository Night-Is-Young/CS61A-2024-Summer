...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        assume_time,assume_money=0,self.balance
        while assume_money<amount:
            assume_time+=1
            assume_money*=(1+self.interest)
        return assume_time
        "*** YOUR CODE HERE ***"
...
class FreeChecking(Account):
    ...
    "*** YOUR CODE HERE ***"
    def withdraw(self, amount):
        current_fee=self.withdraw_fee if self.free_withdrawals<=0 else 0
        self.free_withdrawals-=1
        if amount > self.balance-current_fee:
            return "Insufficient funds"
        if amount > self.max_withdrawal:
            return "Can't withdraw that amount"
        self.balance=self.balance-current_fee-amount
        return self.balance
...
class Transaction:
    ...
    def report(self):
        ...
        if self.changed():
            "*** YOUR CODE HERE ***"
            if self.before>self.after:change='decreased'
            else:change='increased'
            return f'{self.id}: {change} {self.before}->{self.after}'
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        if result==None:
            ...
            if coin not in self.coins:self.coins[coin]=1
