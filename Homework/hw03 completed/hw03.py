...
def pascal(row, column):
    ...
    """
    "*** YOUR CODE HERE ***"
    if column>row or column<0:return 0
    elif column==0 or column==row:return 1
...
def insert_items(s, before, after):
    ...
    for i in range(len(s)):
        if s[i]==before:location.append(i+cnt);cnt+=1
    for i in location:
        s[i+1:i+1]=[after]
    return s
...
def balanced(m):
    ...
    return (length(left_arm)*total_mass(end(left_arm)) ==
            length(right_arm)*total_mass(end(right_arm)) and
...
def berry_finder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    return True in [berry_finder(_) for _ in branches(t)] or label(t)=='berry'
...
def max_path_sum(t):
    ...
    else:return max([max_path_sum(_) for _ in branches(t)])+label(t)
...
def move_stack(n, start, end):
    ...
    "*** YOUR CODE HERE ***"
    assist=6-start-end
    if n==1:print_move(start,end);return
    move_stack(n-1,start,assist)
    print_move(start,end)
    move_stack(n-1,assist,end)
