...
class Player:
    ...
    def debate(self, other):
        ...
        result=50 if self.random_func()<max(0.1, p1 / (p1 + p2)) else -50
        self.popularity=max(0,self.popularity+result)
...
def add_d_leaves(t, v):
    ...
    """
    "*** YOUR CODE HERE ***"
    def add_leaves(t,v,depth):
        for i in t.branches:
            add_leaves(i,v,depth+1)
...
def level_mutation_link(t, funcs):
	...
	if t.is_leaf() and funcs.rest is not Link.empty:
...
def crispr_gene_insertion(lnk_of_genes, insert):
    ...
    """
    "*** YOUR CODE HERE ***"
    def gene_insert(gene,index,flag):
        if gene is Link.empty or index==0:
            return gene
        if flag:
            return Link(insert,gene_insert(gene,index-1,flag))
        if gene.first=='AUG':
            return Link(gene.first,gene_insert(gene.rest,index,True))
        return Link(gene.first,gene_insert(gene.rest,index,flag))
    index=1
    while lnk_of_genes is not Link.empty:
        lnk_of_genes.first=gene_insert(lnk_of_genes.first,index,False)
...
def transcribe(dna):
    ...
    return [''.join([dict[dna[i+j]] for i in range(3)]) for j in range(0,len(dna),3)]
